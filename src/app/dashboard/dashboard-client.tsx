...
=======
  ...
  const { data: weatherHistory, isLoading: isWeatherLoading } =
    useRtdbValue<{ [key: string]: WeatherData }>(weatherRef);

  const latestWeather = useMemo(() => {
    if (!weatherHistory) return null;

    const entries = Object.values(weatherHistory);
  ...
  const displayCondition = currentConditionName
    ? weatherConditions[currentConditionName]
    : weatherConditions['Cloudy'];

  const forecastArray: DailyForecast[] | null = useMemo(() => {
    if (
      !latestWeather?.forecast ||
      !latestWeather.forecast.time ||
      !latestWeather.forecast.weathercode ||
      !latestWeather.forecast.temperature_2m_max ||
      !latestWeather.forecast.temperature_2m_min
    ) {
      return null;
    }
